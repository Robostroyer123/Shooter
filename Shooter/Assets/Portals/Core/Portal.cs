...
public class Portal : MonoBehaviour {
    ...
    public void Render () {
        ...
        if (!CameraUtility.VisibleFromCamera (linkedPortal.screen, Camera.main) || !(Application.isPlaying)) {
            return;
        }

        CreateViewTexture ();

        var localToWorldMatrix = Camera.main.transform.localToWorldMatrix;
    ...
    void UpdateSliceParams (PortalTraveller traveller) {
        ...
        bool playerSameSideAsTraveller = SameSideOfPortal (Camera.main.transform.position, traveller.transform.position);
        if (!playerSameSideAsTraveller) {
            sliceOffsetDst = -screenThickness;
        }
        bool playerSameSideAsCloneAppearing = side != linkedPortal.SideOfPortal (Camera.main.transform.position);
        ...
        for (int i = 0; i < traveller.originalMaterials.Length; i++) {
            traveller.originalMaterials[i].SetVector ("_SliceCentre", slicePos);
